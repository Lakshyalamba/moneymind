...
function Transactions() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3333/api/transactions', {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this transaction?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`http://localhost:3333/api/transactions/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchTransactions();
      } catch (error) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:3333/api/transactions/${editingTransaction.id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchTransactions();
